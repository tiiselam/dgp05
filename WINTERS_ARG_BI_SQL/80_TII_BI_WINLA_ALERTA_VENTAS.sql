...
/****** Object:  View [dbo].[TII_BI_WINLA_ALERTA_VENTAS]    Script Date: 1/14/2020 11:39:44 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[TII_BI_WINLA_ALERTA_VENTAS]
AS
SELECT  'CONTABILIZADA' ESTADO, 
		...
		CASE A.SOPTYPE
				...
				WHEN 3 THEN CASE SUBSTRING(A.SOPNUMBE,1,3)
								WHEN 'TKD ' THEN RTRIM(SUBSTRING(A.SOPNUMBE,1,3)) + '-Nota de Débito'
								ELSE RTRIM(SUBSTRING(A.SOPNUMBE,1,3)) + '-Factura'
								END
				WHEN 4 THEN RTRIM(SUBSTRING(A.SOPNUMBE,1,3)) + '-Nota de Crédito'
		...
		I.USCATVLS_4 DIVISION,
		I.USCATVLS_6 TIPO, 
		RTRIM(LTRIM(B.SLPRSNID)) PAIS,
		...
		CASE WHEN A.SOPTYPE = 4 
				...
				ELSE (C.OXTNDPRC - C.ORTDISAM) END PRECIO, 
		CASE WHEN A.SOPTYPE = 4 
				THEN - (C.OREXTCST) 
                ELSE C.OREXTCST END COSTO, 
...
AND		A.SOPNUMBE NOT LIKE 'RX%' 
AND		A.VOIDSTTS = 0 
AND		((A.SOPTYPE = 3  AND LEFT(A.SOPNUMBE, 3) IN ('TKT', 'TKC', 'TKD') 	AND 
		A.SOPNUMBE NOT LIKE 'RX%' AND 
		A.SOPNUMBE NOT LIKE 'FV %-T00%') OR  (A.SOPTYPE != 3))
AND     CONVERT(CHAR(8),A.DOCDATE ,112) >= 20180401
