...
/****** Object:  View [dbo].[TII_BI_WIN_ALERTA_VENTAS]    Script Date: 5/14/2019 3:43:51 PM ******/
SET ANSI_NULLS ON
GO
...
SELECT  'CONTABILIZADA'                             ESTADO
	...
	,	CASE WHEN DATEDIFF(m, Y1.FSTFSCDY, LEFT(CONVERT(CHAR(8), getdate() , 112), 6) + '01')+1 > 12 THEN 12
			ELSE DATEDIFF(m, Y1.FSTFSCDY, LEFT(CONVERT(CHAR(8), getdate() , 112), 6) + '01')+1
		END  CANT_MESES
	,	CASE WHEN A.SOPTYPE = 4 THEN -(C.XTNDPRCE-C.TRDISAMT) 
			ELSE (C.XTNDPRCE-C.TRDISAMT)
		END PRECIO
	,   CASE WHEN A.SOPTYPE = 4 THEN -(C.EXTDCOST)
			ELSE C.EXTDCOST
		END COSTO
	,   CASE WHEN C.REMPRICE != 0 THEN C.REMPRICE-ROUND((C.TRDISAMT/C.QUANTITY)*C.QTYREMAI, 2) 
			ELSE	0
		END PRECIO_PEND
    ,   CASE WHEN C.REMPRICE != 0 THEN ROUND((C.EXTDCOST/C.QUANTITY)*C.QTYREMAI, 2) 
			ELSE 0 
		END COSTO_PEND
	,	Y1.FSTFSCDY
	,	Y1.LSTFSCDY
